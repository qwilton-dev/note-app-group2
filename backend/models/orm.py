...
class Task(Base):
    ...
    list_id: Mapped[str | None] = mapped_column(
        String, ForeignKey("lists.id", ondelete="SET NULL"), nullable=True, index=True
    )
    ...
    order: Mapped[float] = mapped_column(
        Float,
        nullable=False,
        default=lambda: datetime.now(timezone.utc).timestamp() * 1000,
    )
    estimated_hours: Mapped[float | None] = mapped_column(Float, nullable=True)

    user: Mapped["User"] = relationship("User", back_populates="tasks")
    list: Mapped["List | None"] = relationship("List", back_populates="tasks")
    roadmap: Mapped["Roadmap | None"] = relationship("Roadmap", back_populates="tasks")
    steps: Mapped[list["Step"]] = relationship(
        "Step", back_populates="task", order_by="Step.position"
    )
